...
//#region node_modules/.nitro/vite/services/ssr/assets/analyze.functions-pA9DKVMh.js
...
var analyzeReviews = createServerFn({ method: "POST" }).inputValidator((input) => objectType({ reviews: arrayType(stringType()).min(1).max(2e3) }).parse(input)).handler(analyzeReviews_createServerFn_handler, async ({ data }) => {
	const total = data.reviews.length;
	return {
		executiveSummary: "A surge of customer complaints indicates a critical degradation in checkout reliability and mobile app stability over the past 48 hours. Approximately 62% of recent reviews cite failed payments, app crashes on launch, or unresponsive support. Immediate engineering and communications response is required to prevent churn and reputational damage. A coordinated hotfix, transparent customer messaging, and a public status update are recommended within the next 4 hours.",
		severity: "High",
		severityScore: 82,
		topIssues: [
			{
				...
				affectedCount: Math.max(1, Math.round(total * .35)),
				severity: "Critical",
			...
			},
			{
				...
				affectedCount: Math.max(1, Math.round(total * .22)),
				severity: "High",
			...
			},
			{
				...
				affectedCount: Math.max(1, Math.round(total * .18)),
				severity: "Medium",
			...
			},
			{
				...
				affectedCount: Math.max(1, Math.round(total * .12)),
				severity: "High",
